...
const wishList = ['idd008', 'idd035', 'idd073', 'idd088', 'idd099'];
...
const generateGoodsPage = ()=>{
  ...
  const generateCards = data=>{
    ...
    if(!data.length){
      ...
      goods.textContent = window.location.search === '?wishlist' ? 'Список желаний пуст' : 'По запросу ничего не найдено';
    }
  ...
  if(window.location.pathname.includes('goods') && window.location.search){
